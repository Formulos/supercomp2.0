...
int main ()
{
   ...
   double *x_vector;
   x_vector = new double[num_trials];
   double *y_vector;
   y_vector = new double[num_trials];

   for (i=0;i<num_trials; i++){
      y_vector[i] = distribution(generator);
      x_vector[i] = distribution(generator);
   }

   //seed(-r, r);  // The circle and square are centered at the origin
   double time = omp_get_wtime();

   //#pragma omp parallel for private(test,x,y) reduction(+ : Ncirc)
   for(i=0;i<num_trials; i++)
   {
         x = x_vector[i]; 
         y = y_vector[i];

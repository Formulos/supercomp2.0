...
int main ()
{
   ...
   const int divided = ceil(num_trials/omp_get_max_threads());

   #pragma omp parallel private(test,x,y,i) 
   {
      int current_thread = omp_get_thread_num();
      std::default_random_engine generator (seed);
      std::uniform_real_distribution<double> distribution(-r,r);

      int end = (current_thread+1)*divided;
      int start = (current_thread)*divided;
      long Ncirc_local = 0;
